...
engine = create_async_engine("sqlite+aiosqlite:///:memory:",
                             echo=True
                             )  # echo показывает в консоли работу пакета и генерируемые sql-query
...
async def init():
    async with engine.begin() as conn:
        ...
        await conn.run_sync(Base.metadata.create_all)  # DB creature
...
async def main():
    ...
    async with DBSession() as session:
        async with engine.begin():
            ...
            session.add(n_user)  # это не асинхронная операция, эта операция просто добавляется какое-то хранилище
            ...
            session.add(n_user)  # это не асинхронная операция, эта операция просто добавляется какое-то хранилище
            n_address = Address(email='pupkin@asd.asd', user=n_user)
            session.add(n_address)
            # await session.commit()
